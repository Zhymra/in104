       ...
       Q = (double****)malloc((rows) * sizeof(double***));
        ...
        for (int i = 0; i < rows; i++)
        {
            ...
            for (int j = 0; j < cols; j++)
            {
                ...
                {
                    Q[i][j][k] = (double*)malloc(number_actions * sizeof(double));
                    if (Q[i][j][k] == NULL)
                    {
                        fprintf(stderr, "Out of memory");
                        exit(0);
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                for (int k = 0; k < etages ; k++) 
                    {
                    for (int l = 0; l < number_actions; l++) {
                            Q[i][j][k][l] = 0;
                    }
                }
            }
            ...
            for (int u=0;u<nombre_essais;++u) { 
                ...
                while (nouvel_etat.done!=1 && i<nombre_coups_max) {
                    if (u<nombre_essais-1) {
                        ...
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][prochaine_action] = 
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][prochaine_action] +
                        alpha*(nouvel_etat.reward + gamma_temp*max_Q(state_row_true(state_row),state_col,state_etage) - 
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][prochaine_action]);
                        i=i+1;
                    }
                    ...
                    /*sur le dernier essai on n'admet plus de hasard et on prend le meilleur chemin selon le tableau Q*/
                        ...
                        epsilon=0;
                        printf("%d , %d , %d et ensuite l'action : ", state_row_true(state_row), state_col, state_etage);
            ...
            for (int u=0;u<nombre_essais;++u) { 
                ...
                while (nouvel_etat.done!=1 && i<nombre_coups_max) {
                    if (u<nombre_essais-1) {
                        ...
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][ancienne_action] = 
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][ancienne_action] +
                        alpha*(nouvel_etat.reward + gamma_temp*Q[state_row_true(state_row)][state_col][state_etage][prochaine_action]- 
                        Q[state_row_true(ancienne_ligne)][ancienne_col][ancien_etage][ancienne_action]);        
