...
action epsi_greed(){
    ...
    if (ran_nb>epsilon) {
        ...
        for (int i=0;i<number_actions;++i){
            if (Q[state_row_true(state_row)][state_col_true(state_col)][state_etage][state_etage4][i]>max){
                max = Q[state_row_true(state_row)][state_col_true(state_col)][state_etage][state_etage4][i];
...
action Bolt(){
  ...
  double expo_total = 0;
  for (int i=0;i<number_actions;++i){
    proba[i] = exp(Q[state_row_true(state_row)][state_col_true(state_col)][state_etage][state_etage4][i]);
